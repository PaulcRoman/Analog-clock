const ore = document.querySelector(".hours");
const min = document.querySelector(".minutes");
const sec = document.querySelector(".seconds");

function getTime() {
    const timp = new Date();
    const seconds = timp.getSeconds();
    const minutes = timp.getMinutes();
    const hours = timp.getHours();
    const timeInterval = 6;

    sec.style.transform = 'rotate('+ (seconds * timeInterval)+ 'deg)'
    min.style.transform = 'rotate('+ (minutes * timeInterval + seconds / 10)+ 'deg)'
    ore.style.transform = 'rotate('+ (hours * 30 + minutes / 2)+ 'deg)'
